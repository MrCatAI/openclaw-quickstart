...
$script:QUICKSTART_BASE_URL = if (-not [string]::IsNullOrWhiteSpace($env:OPENCLAW_QUICKSTART_BASE_URL)) {
    $env:OPENCLAW_QUICKSTART_BASE_URL.TrimEnd('/')
} else {
    "https://openclaw.ai"
}
$script:WEB_CONFIG_URL = if (-not [string]::IsNullOrWhiteSpace($env:OPENCLAW_WEB_CONFIG_URL)) {
    $env:OPENCLAW_WEB_CONFIG_URL
} else {
    "$($script:QUICKSTART_BASE_URL)/web-config.js"
}
